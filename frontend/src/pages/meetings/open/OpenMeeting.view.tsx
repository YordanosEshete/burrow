        ...
        <main className="min-h-screen">
            <section className="relative isolate">
                <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-8">
                    <div
                        className="rounded-3xl border border-amber-200/70 bg-gradient-to-r from-amber-100 via-white to-sky-100 p-6 shadow-[inset_0_1px_0_rgba(255,255,255,.6),0_10px_30px_rgba(17,24,39,.06)]">
                        <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                            <div>
                                <div
                                    className="inline-flex items-center gap-2 rounded-full border border-amber-200 bg-amber-50 px-3 py-1 text-xs font-semibold text-amber-700">
                                    {meeting.kind}
                                </div>
                                <h1 className="mt-3 text-2xl md:text-3xl font-bold tracking-tight text-gray-900">{meeting.title}</h1>
                                <p className="mt-1 text-sm text-gray-600">{formatDateTime(meeting.beginningTime, meeting.endTime)}</p>

                                <p className="mb-2 mt-1 text-sm text-gray-500">
                                    Hosted by <span className="font-medium text-gray-700">{data.meetingAuthor}</span>
                                </p>
                                ...
                                <button
                                    onClick={() => nav(`/meeting/${meeting.id}`)}
                    ...
                    <div className="mt-8 grid grid-cols-1 gap-6 lg:grid-cols-12">
                        ...
                        <div className="space-y-6 lg:col-span-4">
                            <section className="rounded-2xl border border-gray-200 bg-white p-5 shadow-sm">
                                <h3 className="text-sm font-semibold text-gray-700">About this meeting</h3>
                                <p className="prose mt-2 max-w-none text-gray-900">{meeting.description}</p>
                            </section>
