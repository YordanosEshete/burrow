...
export default function Meeting() {
    ...
    const isOwner = useMemo(
        () => auth !== "" && user !== null && user.googleID === data?.meeting?.owner,
                        ...
                        <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                            <div className="min-w-0">
                                ...
                                <p className="mb-2 mt-1 text-sm text-gray-500">
                                    Hosted by <span className="font-medium text-gray-700">{meetingAuthor}</span>
                                </p>
                            ...
                            <div
                                className="flex flex-row justify-center items-center gap-2 sm:flex-row sm:items-center">
                                {isOwner && (
                                ...
                                <BookmarkMeeting isBookmarked={data?.bookmarked === true} meetingId={id}/>
                            </div>
                        </div>

                        <div
                            className={`mt-4 flex items-center ${data?.membership?.status === "JOINED" ? "justify-between" : "justify-end"}`}>
                            {data?.membership?.status === "JOINED" && (
                                <button
                                    onClick={() => nav(`/meeting/${meeting.id}/open`)}
